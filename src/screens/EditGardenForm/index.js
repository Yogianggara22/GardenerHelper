...
const EditGardenForm = ({route}) => {
      ...
      .onSnapshot(documentSnapshot => {
        ...
        if (blogData) {
          ...
          setBlogData({
            ...
            categoryName: {
              id: blogData.categoryName.id,
              name: blogData.categoryName.name,
            },
          });
        ...
        setLoading(false); // Move setLoading(false) here to handle the case when blogData is retrieved
      });
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await firestore().collection('blog').doc(itemID).update({
        name: blogData.name,
      ...
      navigation.navigate('GardenDetail', {itemID});
    } catch (error) {
      console.log(error);
    }
  };
            ...
            {dataCategory.map((item, index) => {
              ...
              return (
                <TouchableOpacity
                  key={index}
                  onPress={() =>
                    handleChange('categoryName', {id: item.id, name: item.name})
                  }
